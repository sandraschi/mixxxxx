...
class VideoDecoder : public QThread {
  ...
  public:
    ...
    /// Deck index (1-based) derived from the group, or -1 if the group is not
    /// a mixable deck (e.g. "[Master]", "[Sampler1]", "[PreviewDeck1]").
    int deckIndex() const { return m_deckIndex; }
  ...
  private:
    ...
    /// Pixel format the current SwsContext was built for. Compared against each
    /// incoming frame so hardware-downloaded frames (NV12) and non-4:2:0 sources
    /// are converted correctly instead of being reinterpreted as YUV420P.
